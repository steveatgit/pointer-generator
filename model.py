...
class SummarizationModel(object):
    ...
    def _add_placeholders(self):
        ...
        self._embedding_placeholder = tf.placeholder(tf.float32, [vsize, hps.emb_dim], name='embedding_placeholder')
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                #self._embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], trainable=False)
                self._embedding = tf.get_variable('embedding', [vsize, hps.emb_dim])
                if hps.mode=="train": self._add_emb_vis(self._embedding) # add to tensorboard
                emb_enc_inputs = tf.nn.embedding_lookup(self._embedding, self._enc_batch) # tensor with shape (batch_size, max_enc_steps, emb_size)
                emb_dec_inputs = [tf.nn.embedding_lookup(self._embedding, x) for x in tf.unstack(self._dec_batch, axis=1)] # list length max_dec_steps containing shape (batch_size, emb_size)
    ...
    def run_embedding(self, sess, np_emb):
        sess.run(self._embedding_init, feed_dict={self._embedding_placeholder: np_emb})
